...
void entity::glowesp(DWORD glowObj, int glowInd, float r, float g, float b, float a)
{
	...
	writeMem<float>((glowObj + ((glowInd * 0x38) + 0x24)), true);
	writeMem<float>((glowObj + ((glowInd * 0x38) + 0x25)), false);
}
...
D3DXVECOTR3 entity::getEntBonePos(DWORD base, int boneID)
{
...
{
	return readMem<D3DXVECOTR3>(base + hazedumper::netvars::m_vecOrigin);
}
